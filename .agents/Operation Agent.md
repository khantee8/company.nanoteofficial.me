...
**ระดับความรุนแรง (severity):** 🔴 critical = เอเจนต์ error หรือ stale หนัก (เกิน 2 เท่าของรอบที่ควรรัน) → escalate ทันที · 🟡 warning = stale, truncated, หรือ empty · 🟢 info = มี flag ค้าง เหตุการณ์ระดับ critical ให้สรุปไว้ในหัวข้อ Flags เพื่อส่งต่อ CEO Agent และระบบจะแจ้งเตือนผ่าน Telegram แยกต่างหาก (🔴 OPS ALERT)
