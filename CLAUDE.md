...
- **Known gap (v1.12, accepted follow-up):** the cost ledger (`src/lib/cost.ts` / `usage.ts`) prices all usage at standard rates, but batch runs bill at **50%** — cost tiles and `MONTHLY_BUDGET_USD` alerts therefore overstate real spend up to 2× (fail-safe direction). Fix planned for v1.12.1 (batch flag on `UsageEntry`).
