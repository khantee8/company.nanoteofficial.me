...
**Current version: 1.7.0** (`package.json` — the NavBar reads it). The architecture is the cumulative result of every feature line from the **v1.3 foundation** through **v1.7**; the current release is described just below, and the full per-version history (v0.1.0 → current) lives in **`CHANGELOG.md`**. The load-bearing invariants are restated in **Key Constraints**.

**v1.7.0 (current) — Operations internal monitor.** The Operations agent now monitors the company's own agents, not just CI/CD: on each run it assesses every other agent's run-health — `error` / stale-vs-cadence / truncated (`incomplete`) / empty output / open flags — via a new pure module `src/lib/agents/health.ts` (`assessCompanyHealth()`, `overallSeverity()`, `criticalAlerts()`, `formatHealth()`, and `EXPECTED_CADENCE_HOURS`, which mirrors the per-agent schedule in `vercel.json`). It emits a deterministic **`agent health` scorecard** + an **`agent issues` table** alongside its existing deployment-health output, leads its summary with the **worst severity** found, and routes detected issues into `## Flags` so they propagate into the CEO weekly digest. A **critical** finding additionally fires a distinct **`🔴 OPS ALERT`** Telegram message, separate from the normal run notice. (Known quirk: the 96h rnd/mkt cadence bound can trip a transient 🟡 stale *warning* on a normally skipped day — warning only, never a false critical alert.) See `docs/superpowers/specs/2026-06-14-v17-ops-internal-monitor-design.md`.

_Earlier releases (**v0.1.0 → v1.6.0**) are summarized in [`CHANGELOG.md`](./CHANGELOG.md)._
