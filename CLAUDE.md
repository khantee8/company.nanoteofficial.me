...
**v1.4.7 (current) — Finance research-failure visibility.** Fix: a Finance run that *completed* (`stop_reason: end_turn`) but produced **zero citation-backed funds** was being stored looking clean (empty `summary`, no artifacts). The usual cause is the Anthropic **`web_search` server tool getting rate-limited mid-run** — the throttle surfaces as tool errors inside an otherwise-successful API call, so the model writes uncited funds that `parseFinanceFindings()` then drops to nothing. `completeRaw()`'s retry loop can't catch this (the HTTP request succeeds; only the tool calls fail). `finance.ts` now flags such a run **`incomplete`** (`stopReason === 'max_tokens' || noCitedFunds`) with an explicit Thai summary ("⚠️ ไม่พบกองที่อ้างอิงได้… การค้นเว็บอาจติด rate limit"), so it stays gated as a `draft` and the Telegram notice warns. `runner.ts`'s warning was generalized from "รายงานถูกตัด (max_tokens)" to "รายงานอาจไม่สมบูรณ์" to cover both truncation and research-failure. Known follow-ups (not yet done): `completeRaw()` doesn't resume server-tool `pause_turn` (10-iteration limit); the rate limit itself is an Anthropic account-tier throttle, not a code fix.
